...
csv_writer.writerow(['Stock Name', 'Current Price', 'Previous Close', 'Open', 'Bid', 'Ask', 'Day Range', '52 Week Range', 'Volume'])
...
for url in urls:
	...
	soup= BeautifulSoup(html_page.content, 'lxml') #(page_content, parser)

	header_info= soup.find_all("div", id="quote-header-info")[0]
	stock_title= header_info.find("h1").get_text() #(tag, tag_id)
	current_price= header_info.find("div", class_="My(6px) Pos(r) smartphone_Mt(6px)").find("span").get_text() #(tag, tag_id)
